defmodule <%= base %>Web.PasswordResetControllerTest do
  ...
  test "reset password succeeds for correct key", %{conn: conn} do
    ...
    reset_conn = put(conn, Routes.password_reset_path(conn, :update), password_reset: valid_attrs)<%= if api do %>
    ...
    conn = post(conn, Routes.session_path(conn, :create), session: @update_attrs)<%= if api do %>
